...
def show_restock_dialog(order):
  ...
  def restock_fragment():
    ...
    with st.container(border=True):
      ...
      # Data mapping for renamed columns (Total Price, Deli Fee, Total Cost)
      ...
      for itm in raw_items:
        formatted_items.append({
            ...
            "Qty": itm.get("Qty", 0),
            "Total Price": itm.get("Total Price", itm.get("Price", 0)),
            "Deli Fee": itm.get("Deli Fee", itm.get("Cargo", 0)),
        })

      df_items = pd.DataFrame(formatted_items)
      if "Total Price" not in df_items.columns:
        df_items["Total Price"] = 0
...
# 3. New Restock Order Dialog (Default: HNB-000, New Item, 0, 0, 0)
  ...
  for r_ord in st.session_state.restock_orders:
    for itm in r_ord.get("items", []):
      all_restock_items.append({
          ...
          "Qty": itm.get("Qty", 0),
          "Total Price": itm.get("Total Price", itm.get("Price", 0)),
          "Deli Fee": itm.get("Deli Fee", itm.get("Cargo", 0.0)),
      })
...
if filtered_orders:
  ...
  for index, ord_data in enumerate(filtered_orders):
    ...
    if not items_df.empty:
      tp_col = "Total Price" if "Total Price" in items_df.columns else "Price"
      df_tp = pd.to_numeric(items_df[tp_col], errors="coerce").fillna(0)
      df_qty = pd.to_numeric(items_df["Qty"], errors="coerce").fillna(0)
      
      deli_col = "Deli Fee" if "Deli Fee" in items_df.columns else ("Cargo" if "Cargo" in items_df.columns else "Deli Fee")
